...
reader = pd.read_table(Data_Directory+'/Data/question_train_set.txt',sep='\t',header=None)
...
topic_reader = pd.read_table(Data_Directory+'/Data/question_topic_train_set.txt',sep='\t',header=None)
...
# 导入topic_info
label_reader = pd.read_table(Data_Directory+'/Data/topic_info.txt',sep='\t',header=None)
...
# 保存处理过后的文件
data_topic.to_csv(Data_Directory+'/Data/data_topic.txt', header=None, index=None, sep='\t')

# 切分成10块保存
for i in xrange(10):
    data_topic_filename = Data_Directory + '/Data/data_topic_block_' + str(i) + '.txt'
